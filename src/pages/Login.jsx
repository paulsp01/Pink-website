...
const Login = ({showloginpopup,setShowloginpopup}) => {
 ...
 setShowloginpopup(false)
  ...
  return (
    <div className='relative w-100 h-auto shadow-md bg-white py-6 flex flex-col justify-between items-center rounded-md'>
        <h1 className='text-3xl text-pink-600 font-bold'>Login</h1>
         <button onClick={handleloginClose} className=' text-gray-500 border border-purple-600 rounded-full w-8  h-8 cursor-pointer absolute right-6 top-4 text-sm'>X</button>
        <form >
        ...
        <button onClick={handleloginClose} className='bg-purple-600 text-white my-4 px-6 py-2 font-semibold rounded-md'>Login</button>
        <p className='text-xs text-purple-600'>Don't have an account? <span className='text-pink-600'>Click here</span></p>
    </div>
  )
}
